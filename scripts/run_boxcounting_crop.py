...
def process_image(model_path, image_path, output_dir,  conf_threshold, show_disp):
    ...
    boxcounting = BOXCOUNTING(model_path,show=False, conf_threshold=0.3)
    ...
    for cls, clusters in clusters_by_class.items():
        for cluster in clusters:
            ...
            lr_margin = 60
            td_margin = 10
            if y1 < td_margin or x1 < lr_margin or y2 > h-td_margin or x2 > w-lr_margin:
                continue

            if y1s < y2s and x1s < x2s:

                # Create a black mask with the same dimensions as the frame
                # mask = np.zeros_like(frame)

                # Fill the ROI in the mask with white color
                # mask[y1s:y2s, x1s:x2s] = frame[y1s:y2s, x1s:x2s]

                # Apply the mask to the frame
                # masked_frame = cv2.bitwise_and(frame, mask)
                # cv2.imshow('YOLOv8 ss', cv2.resize(masked_frame, (1080, 720)))
                # result = boxcounting.count(masked_frame)

                cropped_frame =image[y1s:y2s, x1s:x2s]
                if cropped_frame.size > 0:
                    cv2.imshow('YOLOv8 ss', cropped_frame)
                    result = boxcounting.count(cropped_frame)

            cv2.rectangle(image, (x1s, y1s), (x2s, y2s), (0, 255, 0), 2)

            color = colors[cls % len(colors)]
            cv2.rectangle(image, (x1, y1), (x2, y2), color, 5)
            cluster_label = f"{Class[cls]}:{len(cluster)}:{result}"
            ...
            # Draw the text on top of the rectangle
            cv2.putText(image, cluster_label, (x1, label_y2 - baseline), cv2.FONT_HERSHEY_SIMPLEX, 1, (255, 255, 255), 2)
...
def process_video(model_path, video_path, skip_frames, output_dir,  conf_threshold, show_disp):
    ...
    model = YOLO(model_path)
    boxcounting = BOXCOUNTING(model_path,show=False, conf_threshold=0.5)
    # Open the video file
        ...
        for cls, clusters in clusters_by_class.items():
            for cluster in clusters:
                ...
                if y1s < y2s and x1s < x2s:
                    ...
                    cropped_frame = frame[y1s:y2s, x1s:x2s]
                    if cropped_frame.size > 0:
                        cv2.imshow('YOLOv8 ss', cropped_frame)
                        result = boxcounting.count(cropped_frame)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--model", help="Path to the YOLOv8 model file.", default='models/v13.pt')
    parser.add_argument("--source", help="Path to the video or image file or folder containing files.", default=r'inputs\2024-05-21-KX11_382_3742A_1_of_1_processed.MP4')
    parser.add_argument("--output", help="Path for output.", default='outputs/Osot-demo')
    ...
    parser.add_argument("--show", type=bool, default=True, help="Display the processed images and videos.")
